...
describe('Reading XAdES', () => {
  it('X_AT_SIT_1', async () => {
    ...
    assert.equal(
      xades.Properties.SignedProperties.SignedSignatureProperties.SigningTime.Value instanceof Date,
      true,
    );
    assert.equal(
      xades.Properties.SignedProperties.SignedSignatureProperties.SigningTime.Value.getFullYear(),
      2016,
    );
    assert.equal(
      xades.Properties.SignedProperties.SignedSignatureProperties.SigningCertificate.Count,
      1,
    );
    assert.equal(
      xades.Properties.SignedProperties.SignedSignatureProperties.SigningCertificate.Item(0)
    ...
    assert.equal(
      xades.Properties.SignedProperties.SignedDataObjectProperties.DataObjectFormats.Count,
      1,
    );
    assert.equal(
      xades.Properties.SignedProperties.SignedDataObjectProperties.DataObjectFormats.Item(0)
        ?.ObjectReference,
      '#r-id-1',
    );
    assert.equal(
  ...
  it('X_BE_CONN_10', async () => {
    ...
    assert.equal(
      xades.Properties.SignedProperties.SignedSignatureProperties.SigningCertificate.Item(0)
        ?.IssuerSerial.X509SerialNumber,
      '288350169419475868349393263988944744462',
    );
    assert.equal(
      xades.Properties.SignedProperties.SignedSignatureProperties.SigningCertificate.Item(0)
        ?.CertDigest.DigestMethod.Algorithm,
      'http://www.w3.org/2000/09/xmldsig#sha1',
    );
    assert.equal(
      xades.Properties.SignedProperties.SignedSignatureProperties.SigningCertificate.Item(0)
        ?.CertDigest.DigestValue.byteLength,
      20,
    );
    assert.equal(
      xades.Properties.SignedProperties.SignedSignatureProperties.SigningCertificate.Item(1)
        ?.IssuerSerial.X509IssuerName,
  ...
  it('X_FR_CS_3', () => {
    ...
    const signaturePolicyIdentifier =
    ...
    assert.equal(
      signaturePolicyId.SigPolicyHash.DigestMethod.Algorithm,
    ...
    assert.equal(
      signedDataObjectProperties.CommitmentTypeIndications.Item(0)?.CommitmentTypeId.Identifier
        ?.Value,
      '1.2.840.113549.1.9.16.6.3',
    );
    assert.equal(
      signedDataObjectProperties.CommitmentTypeIndications.Item(0)?.AllSignedDataObjects,
      true,
    );
  });
  ...
  it('X_BG_BOR_1', () => {
    ...
    assert.equal(
      cert?.IssuerSerial.X509IssuerName,
      '2.5.4.20=#13102b333539203220392032313520313030,1.2.840.113549.1.9.1=#161263613571657340622d74727573742e6f7267,2.5.4.17=#130431373834,STREET=bul. Tsarigradsko shose No 117,CN=B-Trust Operational CA QES,OU=B-Trust,O=BORICA - BANKSERVICE AD, EIK 201230426,L=Sofia,ST=Sofia,C=BG',
    );
    ...
    assert.equal(
      ...
      'bul. Tsar Boris 3 41,Sofia,PK:1612,EGN:8205301708',
    );
  ...
  it('X_BE_CONN_26', () => {
    ...
    xades.UnsignedProperties.UnsignedSignatureProperties.Some((item) => {
      if (item instanceof SignatureTimeStamp) {
        ...
        assert.equal(
          item.CanonicalizationMethod.Algorithm,
          'http://www.w3.org/2001/10/xml-exc-c14n#',
        );
        assert.equal(item.EncapsulatedTimeStamp.Count, 1);
        assert.equal(
          item.EncapsulatedTimeStamp.Item(0)?.Id,
          'ETS-e964b0fc-172d-4631-bc0e-fa456c429a5e',
        );
    ...
    xades.UnsignedProperties.UnsignedSignatureProperties.Some((item) => {
      if (item instanceof ArchiveTimeStamp) {
        ...
        assert.equal(
          item.EncapsulatedTimeStamp.Item(0)?.Id,
          'ETS-8081f955-d8c9-498f-8922-dea64a21128d',
        );
        assert.equal(item.EncapsulatedTimeStamp.Item(0)?.Value.byteLength, 4758);
