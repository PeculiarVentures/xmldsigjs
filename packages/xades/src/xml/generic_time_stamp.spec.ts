...
describe('GenericTimeStamp', () => {
  it('Parse', () => {
    const xmlObject = new GenericTimeStamp();

    xmlObject.Id = '123';
    const encTS = new EncapsulatedTimeStamp();
    ...
    const xmlOutput = xmlObject.GetXml();
    assert.ok(xmlOutput);
    c14n.LoadInnerXml(xmlOutput);
    ...
    const xmlObject2 = GenericTimeStamp.LoadXml(xml);
