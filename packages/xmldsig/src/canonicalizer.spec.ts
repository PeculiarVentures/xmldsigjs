...
describe('Canonicalization', () => {
  ...
  describe('Custom', () => {
    ...
    it('#9 Canonicalization works on xml with multiple prefixed namespaces defined in inclusive list', () => {
      ...
      ExcC14N(
        xml,
        xpath,
        '<p:child xmlns:p="ns"><inclusive:inner xmlns:inclusive="ns2">123</inclusive:inner><inclusive2:inner xmlns:inclusive2="ns3">456</inclusive2:inner></p:child>',
      );
      ExcC14N(
        xml,
        xpath,
    ...
    it('#11 Canonicalization works on xml with prefixed namespace defined in inclusive list used on attribute', () => {
      ...
      ExcC14N(
        xml,
        xpath,
        '<p:child xmlns:p="ns"><p:inner foo="inclusive:bar">123</p:inner></p:child>',
      );
      ExcC14N(
        xml,
        xpath,
    ...
    it('#16 Canonicalization works when prefixed namespace is defined in output nodes not in the parent chain of who needs it', () => {
      const xml =
        '<root><child><p:inner1 xmlns:p="foo" /><p:inner2 xmlns:p="foo" /><p:inner3 xmlns:p="foo" /></child></root>';
      const xpath = '//*[local-name(.)="child"]';
      C14N(
        xml,
        xpath,
        '<child><p:inner1 xmlns:p="foo"></p:inner1><p:inner2 xmlns:p="foo"></p:inner2><p:inner3 xmlns:p="foo"></p:inner3></child>',
      );
      ExcC14N(
        xml,
        xpath,
    ...
    it('#37 Canonicalization sorts upper case namespaces before lower case', () => {
      const xml =
        '<root xmlns:a="urn:a.example.com" xmlns:Z="urn:z.example.com" a:a="1" Z:a="2"></root>';
      const xpath = '//*[local-name(.)="root"]';
      C14N(
        xml,
        xpath,
        '<root xmlns:Z="urn:z.example.com" xmlns:a="urn:a.example.com" a:a="1" Z:a="2"></root>',
      );
      ExcC14N(
        xml,
        xpath,
  ...
  describe('Test cases for Canonical XML 2.0', () => {
