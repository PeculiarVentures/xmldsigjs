...
describe('Verify XML signatures', function () {
  async function verifyXML(name: string, res = true) {
    ...
    const signatures = xml.getElementsByTagNameNS(
      'http://www.w3.org/2000/09/xmldsig#',
      'Signature',
    );
  ...
  async function verifyExternalXML(name: string, externalName: string, res = true) {
    ...
    const signature = xmldsig.Select(
      xml,
  ...
  describe('aleksey.com', () => {
    ...
    ].forEach((file) => it(file, async () => verifyXML(`${file}.xml`)));
  });

  [['valid_signature_asic', 'ping']].forEach((file) =>
