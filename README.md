...
[XMLDSIGjs](https://en.wikipedia.org/wiki/XMLDSIG) is short for "XML Digital Signature". This library aims to provide an implementation of both XMLDSIG in Typescript/Javascript that uses Web Crypto for cryptographic operations so it can be used both in browsers and in Node.js (when used with a polyfill like [node-webcrypto-ossl](https://github.com/PeculiarVentures/node-webcrypto-ossl) or [node-webcrypto-p11](https://github.com/PeculiarVentures/node-webcrypto-p11)).
